...
public:
  static client_uploader_s create(boost::asio::io_context &io,
    ...
    client_uploader_s self(new client_uploader(io, file_name, remote_endpoint,
...
private:
  ...
  client_uploader(boost::asio::io_context &io, const std::string &file_name,
                  const udp::endpoint &remote_endpoint,
                  std::unique_ptr<std::istream> u_in_stream,
                  client_completion_callback download_callback);
...
public:
  ...
  void upload_file(const std::string &remote_file_name,
                   std::string local_file_name,
                   client_completion_callback upload_callback) {
    client_uploader::create(
        this->io, remote_file_name, this->remote_endpoint,
        std::make_unique<std::ifstream>(local_file_name,
  ...
  void upload_file(const std::string &remote_file_name,
    ...
    client_uploader::create(this->io, remote_file_name, this->remote_endpoint,
                            std::move(u_in_stream), upload_callback);
  }
