...
void download_client::receive_data() {
  ...
  this->receiver.async_receive(this->network_frame.get_asio_buffer_for_recv(),
...
void download_client::receive_data_cb(const boost::system::error_code &error,
  ...
  try {
    ...
    WARN("Failed to parse data response from %s", to_string(this->receiver.get_receive_endpoint()).c_str());
    this->receive_data();
    return;
  }
  ...
  if (this->block_number + 1 != this->network_frame.get_block_number()) {
    ...
    this->send_ack_for_block_number(this->network_frame.get_block_number());
    return;
  }
  this->block_number = this->network_frame.get_block_number();
  if (!this->write_handle.write_buffer(this->network_frame.data_cbegin(), this->network_frame.data_cend())) {
...
void upload_client::receive_ack_cb(const boost::system::error_code &error, const std::size_t bytes_received) {
  ...
  try {
    ...
    if (this->network_frame.get_op_code() == frame::op_error) {
      XDEBUG("Server responded with error :%s", this->network_frame.get_error_message().c_str());
      this->exit(this->network_frame.get_error_code());
      return;
    } else {
      this->receive_ack();
    }
...
void upload_client::send_data(bool resend) {
  ...
  this->sender.async_send(this->network_frame.get_asio_buffer(), this->server_tid, send_data_cb);
}
